...
class BaseEnv(gym.Env):
    def __init__(self, map_type, visualization, agent_index=None):
    ...
    def _get_init_map(self, index=None):
        ...
        if self.map_type == "test":
            with open('/home/avsp/Masaüstü/GansNRoses/dqn/library/all_test_maps.pkl', 'rb') as f:
                map_dataset = pickle.load(f)
            return map_dataset[index].copy()

        elif self.map_type == "train":
            with open('/home/avsp/Masaüstü/GansNRoses/dqn/library/library_maps.pkl', 'rb') as f:
                map_dataset = pickle.load(f)
                map_s = len(map_dataset)
                map_indx = np.random.randint(0, map_s)
            return map_dataset[map_indx][0].copy()

        elif self.map_type == "score":
            with open('/home/avsp/Masaüstü/GansNRoses/dqn/library/temp_map.pkl', 'rb') as f:
                map_dataset = pickle.load(f)
            return map_dataset.copy()

        elif self.map_type == "exp_random":
            #TODO: Modify this part
            with open('/home/avsp/Masaüstü/GansNRoses/dqn/library/random_100_maps.pkl', 'rb') as f:
                map_dataset = pickle.load(f)
                """
                index_list = []
                ...
                for _ in range(10):
                    map_s = len(map_dataset)
                    map_indx = np.random.randint(0, map_s)
                    index_list.append(map_indx)
                """
            return map_dataset[self.agent_index*10+index].copy()

        elif self.map_type == "exp_gan":
            with open('/home/avsp/Masaüstü/GansNRoses/dqn/library/library_maps.pkl', 'rb') as f:
                map_dataset = pickle.load(f)
                map_s = len(map_dataset)
                map_indx = np.random.randint(0, map_s)
...
class CurriculumEnv(BaseEnv):
    ...
    def __init__(self, map_type, visualization, agent_idx=None):
        super(CurriculumEnv,self).__init__(map_type, visualization, agent_index=agent_idx)
    ...
    def reset(self):
        ...
        if self.map_type == "exp_random":
            #TODO: This part can be modified
            if self.index >= 3:
                self.index = 0
            init_map = self.get_init_map(index=self.index)
