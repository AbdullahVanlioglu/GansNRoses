...
from environment.base_env import TestTrapEnv
...
class GAN:
    ...
    def generate_map(self, gen_lib, opt, idx):

        if opt.model == "test":
            self.G.load_state_dict(torch.load(f"./weights/G{idx+1}_trap_300.pth"))
        ...
        while True:
            ...
            if len(gen_lib.train_library) >= 10:
                break
