...
class CustomCNN(BaseFeaturesExtractor):
    ...
    def __init__(self, observation_space: gym.spaces.Box, features_dim: int = 256):
        ...
        self.cnn = nn.Sequential(
            nn.Conv2d(n_input_channels, 128, kernel_size=3, stride=1),
            nn.ReLU(),
        ...
        self.linear = nn.Sequential(
            nn.Linear(n_flatten, int(n_flatten/4)), 
            nn.ReLU(),
            nn.Linear(int(n_flatten/4), int(n_flatten/8)), 
            nn.ReLU(),
            nn.Linear(int(n_flatten/8), features_dim), 
            nn.Sigmoid()
            )
...
def main():
    ...
    for k in range(10):
        for i in range(10):
            print("{} to {} training...".format(k,i))
            vecenv = make_vec_env(lambda: QuadrotorFormation(map_type="train", visualization=False, random_map_start=k*50+1,random_map_end=k*50+5*i+6), n_envs=1, vec_env_cls=SubprocVecEnv)
            model = DQN('CnnPolicy', vecenv, policy_kwargs=policy_kwargs, verbose=0, exploration_fraction=0.85, tensorboard_log=f"./dqn_tensorboard")
