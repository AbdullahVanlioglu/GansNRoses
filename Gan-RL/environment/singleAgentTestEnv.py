...
class TestQuadrotorFormation(gym.Env):
    ...
    def step(self, action):
        ...
        if np.all(self.reward_map == 0) or self.iteration >= 20:
            done = True
            self.close()
        ...
        return state, self.reward, done, solved
    ...
    def get_init_map(self, index):
        ...
        if self.map_type == "train":
            with open('datas/all_possible_maps.pkl', 'rb') as f:
                map_dataset = pickle.load(f) 

        elif self.map_type == "test":
            with open('datas/random_1000_test_maps.pkl', 'rb') as f:
                map_dataset = pickle.load(f)
            ...
            with open('datas/library_maps.pkl', 'rb') as f:
                map_dataset = pickle.load(f)
