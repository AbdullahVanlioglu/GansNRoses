...
class QuadrotorFormation(gym.Env):
    ...
    def step(self, action):
        ...
        if np.all(self.reward_map == 0) or self.iteration >= 300:
            done = True
            self.map_iter += 1
            self.close()
    ...
    def get_init_map(self, index):
        ...
        if self.map_type == "train":
            with open('datas/random_1000_train_maps.pkl', 'rb') as f:
                map_dataset = pickle.load(f) 

        elif self.map_type == "test":
            with open('datas/all_test_maps.pkl', 'rb') as f:
                map_dataset = pickle.load(f)

        elif self.map_type == "random":
            with open('datas/random_1000_maps.pkl', 'rb') as f:
                map_dataset = pickle.load(f)

        elif self.map_type == "gan":
            with open('datas/library_maps_gan5.pkl', 'rb') as f:
                map_dataset = pickle.load(f)
                
            return map_dataset[index][0].copy()
        elif self.map_type == "gan_nice":
            with open('datas/nice_maps.pkl', 'rb') as f:
                map_dataset = pickle.load(f)
    ...
    def reset(self):
        ...
        init_map = self.get_init_map(self.map_index-1)
    ...
    def update_agent_pos(self, discrete_action):
        ...
        self.agent.state = np.zeros((10,10))
