...
class BaseEnv(gym.Env):
    ...
    def _get_init_map(self, index=None):
        ...
        if self.map_type == "test":
            ...
            with open(f'/home/avsp/Masaüstü/GansNRoses/GansNRoses/library/agent{self.agent_idx+1}_300.pkl', 'rb') as f:
                map_dataset = pickle.load(f)
...
class TrapEnv(BaseEnv):
    ...
    def __init__(self, map_type, visualization):
    ...
    def reset(self):
        ...
        if self.map_iter % 5000 == 0 and self.map_iter != 0:
            self.map_index = np.random.randint(1, self.num_of_map)
    ...
    def render(self, mode='human'):
        if self.viewer is None:
            ...
            sprite_dict['T'] = mapsheet.crop((3*16, 2*16, 4*16, 3*16))
...
class TrapTestEnv(BaseEnv):

    def __init__(self, map_type, visualization):
        super(TrapTestEnv, self).__init__(map_type, visualization)
...
class ExperimentalEnv(BaseEnv):

    def __init__(self, map_type, visualization, agent_idx):
        super(ExperimentalEnv,self).__init__(map_type, visualization, agent_idx)
        self.observation_space = spaces.Box(low=0, high=255,
                                        shape=(4, 4, 3), dtype=np.uint8)

        self.trap_map = np.zeros((4, 4))
        self.map_index = 0
        self.max_map_number = 50
        self.random_map_indexes = []

        for i in range(0,self.max_map_number):
            n = random.randint(1,50000)
            self.random_map_indexes.append(n)
    ...
    def reset(self):
        ...
        if self.map_type == "gan":  
            ...
            init_map = self.get_init_map(self.random_map_indexes[self.map_index])

        if self.map_iter % 5000 == 0 and self.map_iter != 0:
            self.map_index += 1
        
        if self.map_index >= self.max_map_number:
            self.map_index = 0
...
class ExperimentalTestEnv(BaseEnv):

    def __init__(self, map_type, visualization, agent_idx):
        super(ExperimentalTestEnv,self).__init__(map_type, visualization, agent_idx)
        self.observation_space = spaces.Box(low=0, high=255,
                                        shape=(4, 4, 3), dtype=np.uint8)
    ...
    def reset(self, idx):
        ...
        init_map = self.get_init_map(idx)
